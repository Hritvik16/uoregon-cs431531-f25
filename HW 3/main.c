...

{
    ...
    for (int i = 0; i < nnz; i++) {
        row_ind[i]--;   // from [1..m] -> [0..m-1]
        col_ind[i]--;   // from [1..n] -> [0..n-1]
    }

    unsigned int* row_ptr = (unsigned int*) calloc(m + 1, sizeof(unsigned int));
    unsigned int* col_out = (unsigned int*) malloc(sizeof(unsigned int) * nnz);
    double*       val_out = (double*)       malloc(sizeof(double) * nnz);

    if (!row_ptr || !col_out || !val_out) {
        fprintf(stderr, "Error: malloc failed in convert_coo_to_csr\n");
        exit(EXIT_FAILURE);
    }
    ...
    unsigned int* next = (unsigned int*) malloc(sizeof(unsigned int) * m);
    if (!next) {
        fprintf(stderr, "Error: malloc failed in convert_coo_to_csr (next)\n");
        exit(EXIT_FAILURE);
    }

    memcpy(next, row_ptr, sizeof(unsigned int) * m);
              ...
              omp_lock_t* writelock)
{
    ...
    for (int i = 0; i < nnz; i++) {
        ...
        double product = vals[i] * vector_x[c];
          ...
          double* vector_x, double *res)
{
    ...
    for (int r = 0; r < m; r++) {
        ...
        for (unsigned int idx = start; idx < end; idx++) {
            int c = (int) csr_col_ind[idx];
            sum += csr_vals[idx] * vector_x[c];
        }
