                        ...
                        int* n_new)
{
    ...
    int max_row_len = 0;
    for(int i = 0; i < m; i++) {
        int len = csr_row_ptr[i+1] - csr_row_ptr[i];
        if(len > max_row_len) max_row_len = len;
    }
    *n_new = max_row_len;

    
    unsigned int* ec = (unsigned int*) malloc(sizeof(unsigned int) * m * max_row_len);
    double* ev = (double*) malloc(sizeof(double) * m * max_row_len);
    assert(ec && ev);
    ...
    for(int i = 0; i < m; i++) {
        ...
        for(int j = 0; j < len; j++) {
            int idx = i * max_row_len + j;
            ec[idx] = csr_col_ind[start + j];
            ev[idx] = csr_vals[start + j];
        }

        
        for(int j = len; j < max_row_len; j++) {
            int idx = i * max_row_len + j;
            ec[idx] = 0;
            ev[idx] = 0.0;
        }
