...
void prefix_sum_p1(int* src, int* prefix, int n)
{
    ...
    for (int k = 1; k < n; k *= 2) {
        ...
        for (int i = k; i < n; i++) {
            ...
            prefix[i] = prefix[i] + prefix[i - k];
        }
    }
    printf("Parallel O(nlogn)" + prefix[n-1]);
}
...
void prefix_sum_p2(int* src, int* prefix, int n)
{
    ...
    #pragma omp parallel for
    for (int i = 0; i < n; i++) {
        prefix[i] = prefix[i] + src[i];
    }
    // printf("Parallel O(2n)" + prefix[n-1]);
}
